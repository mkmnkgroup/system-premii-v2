...
# ZAKŁADKI GŁÓWNE
# ==========================================
tab_gen, tab_calc, tab_dash, tab_history = st.tabs(["📋 Generator Harmonogramu", "🧮 Kalkulator Premii", "📊 Dashboard i Wykresy", "📁 Archiwum Historyczne"])
...
# Panel boczny - Wgrywanie plików z produkcją (Bez pakowania)
st.sidebar.markdown("---")
...
# Panel boczny - Edycja Średnich 12M i Wag Parametrów (Bez pakowania)
st.sidebar.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.subheader("➕ Powody nieobecności")
new_reason_input = st.sidebar.text_input("Dodaj nowy powód:")
if st.sidebar.button("Dodaj do listy"):
    if new_reason_input and new_reason_input.strip():
        ...
        if reason_clean not in st.session_state.absence_reasons:
            ...
            st.sidebar.success(f"Dodano: {reason_clean}")
            st.rerun()
...
with tab_calc:
    ...
    if st.session_state.current_schedule_df.empty:
        ...
        for name, group in df_sched.groupby("OSOBA"):
            ...
            # Wersja v2: Brak potrąceń za nieobecności!
            summary_list.append({
                "Pracownik": name,
