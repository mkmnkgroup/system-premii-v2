...
# Parametry Wyliczeniowe i Nowe Dodatki
...
if "forklift_pool" not in st.session_state:
  st.session_state.forklift_pool = 600.0  # Zbiorcza pula na wózki (zł netto)
if "rate_pallet" not in st.session_state:
  st.session_state.rate_pallet = 10.0  # Rozładunki/Załadunki palet (zł/szt.)
...
with tab_calc:
  ...
  if st.session_state.current_schedule_df.empty:
    ...
    pallet_payout_total = cur_pallets * st.session_state.rate_pallet

    st.info(
        f"💡 Podstawa premii: **{base_salary:,.2f} zł netto** | Mnożnik"
        f" Kierownika: **{st.session_state.kierownik_bonus_multiplier:.2f}x**"
        " | Pula na wózki:"
        f" **{st.session_state.forklift_pool:,.2f} zł netto** | Paleta:"
        f" **{st.session_state.rate_pallet:,.2f} zł/szt.**"
        .replace(",", " ")
        .replace(".", ",")
    )
    ...
    with col_m4:
      st.metric(
          label="Liczba Palet / Pula za Palety",
          value=f"{cur_pallets:.0f} szt. / {pallet_payout_total:,.2f} zł",
      )
    ...
    for day in range(1, days_in_month + 1):
      ...
      daily_work_pct_map[day] = pct_work_d
    ...
    # Tabela edycyjna do wprowadzania godzin pracy na wózku
    forklift_editor_df = st.data_editor(
        st.session_state.employees_df[["OSOBA", "CZAS WÓZKA (godz.)"]],
        column_config={
            "OSOBA": st.column_config.TextColumn(
                "Pracownik", disabled=True
            ),
    ...
    with col_fk2:
      st.metric(
          label="Zbiorcza Pula do Podziału",
    ...
    st.markdown("---")
    st.subheader("👥 4. Szczegółowe Rozliczenie Pracowników")
    ...
    for emp_name in unique_emps:
      ...
      # NALICZANIE DODATKU ZA WÓZEK NA PODSTAWIE UADZIAŁU % Z 100%
      emp_forklift_pct = (
      ...
      total_payout = calculated_bonus + ot_pay + spec_bonus_val + forklift_pay
      ...
      emp_summary.append({
          ...
          "Czas na wózku (h)": emp_forklift_hours,
          "% Udział wózek": f"{emp_forklift_pct:.2f}%".replace(".", ","),
    ...
    # =========================================================
    # IMPREZY / PASKI DO WYDRUKU DLA PRACOWNIKÓW
    # =========================================================
    st.markdown("---")
    st.subheader("🧾 5. Imienne Paski Premiowe do Wydruku")

    col_print_sel, col_print_btn = st.columns([3, 1])
    with col_print_sel:
    ...
    emps_to_render = (
        unique_emps
        if selected_slip_emp == "WSZYSCY PRACOWNICY"
        else [selected_slip_emp]
    )

    for e_name in emps_to_render:
      e_det = emp_details_map.get(e_name)
      if not e_det:
        continue
          ...
          f"""
            <div class="pay-slip">
                <div class="pay-slip-header">
                    <h3 style="margin:0; color:#1e3a8a;">PASEK ROZLICZENIA PREMIOWEGO — {period_key.upper()}</h3>
                    <p style="margin:5px 0 0 0; font-size:16px;"><strong>Pracownik:</strong> {e_name} | <strong>Stanowisko:</strong> {e_det['position']}</p>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                    <div>
                        <p style="margin:3px 0;"><strong>Dni planowane:</strong> {e_det['scheduled_days']}</p>
                        <p style="margin:3px 0;"><strong>Dni przepracowane:</strong> {e_det['days_worked']}</p>
          ...
          f"""
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <p style="margin:2px 0;"><strong>Czas na wózku:</strong> {e_det['emp_forklift_hours']} godz. ({e_det['emp_forklift_pct']:.2f}% udziału w puli {st.session_state.forklift_pool:,.2f} zł)</p>
                        <p style="margin:2px 0;"><strong>Dodatek za wózek widłowy:</strong> {e_det['forklift_pay']:,.2f} zł</p>
                    ...
                    <div style="background-color:#f1f5f9; padding:10px 15px; border-radius:8px; border:1px solid #cbd5e1;">
                        ...
                        <span style="font-size:22px; font-weight:bold; color:#1e3a8a;">{e_det['total_payout']:,.2f} zł netto</span>
                    </div>
                </div>
            </div>
            """,
